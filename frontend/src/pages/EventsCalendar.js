...
const EventsCalendar = () => {
  ...
  const fetchEvents = useCallback(async () => {
    try {
      ...
      const res = await axios.get('http://localhost:/api/events', config);
  ...
  const handleAddEvent = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:/api/events', body, config);
      await fetchEvents();
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`http://localhost:/api/events/${deleteModal.eventId}`, config);
